...
public class Texture {
    ...
    public Texture(BufferedImage image) {
        ...
        for (int x = 0; x < image.getWidth(); x++) {
            for (int y = 0; y < image.getHeight(); y++) {
                ...
                if(color.isPresent()) {
                    temp.put(color.get(), new Vector3d((image.getWidth()-1)-x, (image.getHeight()-1)-y, 0).mul(ratio).sub(0,height*0.5f, 0));
                }
            }
        }
    ...
    public void draw(Viewer viewer, Matrix4d... matrix) {
        for (Map.Entry<ParticleEffect, Collection<Vector3d>> entry : data.asMap().entrySet()) {
            for (Vector3d v : entry.getValue()) {
                Vector4d vec = v.toVector4(1);
    ...
    private static Optional<Color> getColor(int pixel) {
        return Optional.of(new java.awt.Color(pixel, true)).filter(c -> {
            return c.getAlpha() != 0;
        }).map(Color::of);
    }
}
